class LunarDate {
  ...
  String get monthText => _monthNames[month - 1];
  String get dayText => _dayNames[day - 1];
  String get shortText => day == 1 ? '${isLeapMonth ? '闰' : ''}$monthText月' : dayText;
  String get fullText => '农历${isLeapMonth ? '闰' : ''}$monthText月$dayText';
...
class LunarCalendar {
  ...
  static LunarDate fromSolar(DateTime solar) {
    ...
    var lunarYear = 1900;
    var yearDays = _yearDays(lunarYear);
    ...
    while (lunarMonth <= 12 && offset >= 0) {
      if (leap > 0 && lunarMonth == leap + 1 && !isLeap) {
        lunarMonth--;
        isLeap = true;
        monthDays = _leapDays(lunarYear);
      } else {
        monthDays = _monthDays(lunarYear, lunarMonth);
      }
  ...
  static int _yearDays(int year) {
    var sum = 348;
    final data = _lunarInfo[year - 1900];
  ...
  static int _monthDays(int year, int month) {
    return (_lunarInfo[year - 1900] & (0x10000 >> month)) == 0 ? 29 : 30;
  }
}
